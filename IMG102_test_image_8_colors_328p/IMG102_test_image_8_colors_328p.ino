...
void IMG1_Send_framebuffer(uint8_t current_row, uint8_t buffer_offset) {
...
void loop() {
// Прогон снизу-верх
    for (current_row = 99; current_row >= 0 ; current_row--) {
        IMG1_Send_framebuffer(current_row, 0);
        IMG1_Send_framebuffer(current_row, 200);
        IMG1_Send_framebuffer(current_row, 100);
        IMG1_Send_framebuffer(current_row, 200);
        IMG1_Send_framebuffer(current_row, 200);
        IMG1_Send_framebuffer(current_row, 100);
        IMG1_Send_framebuffer(current_row, 200);
